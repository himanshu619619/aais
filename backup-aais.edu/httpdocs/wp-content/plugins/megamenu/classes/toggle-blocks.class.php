...
class Mega_Menu_Toggle_Blocks {
    ...
    public function output_menu_public_toggle_block_html( $html, $settings ) {
        ...
        if ( ! $css_version || version_compare($css_version, '2.4.0.2') >= 0 ) {
			...
			if(ICL_LANGUAGE_CODE == 'zh-hans') { $chtoggletext = '目录'; } else { $chtoggletext = 'MENU'; }

            $html = "<span class='mega-toggle-label'><span class='mega-toggle-label-closed'>{$chtoggletext}</span><span class='mega-toggle-label-open'>{$chtoggletext}</span></span>";
        } else {
            $html = "";
        }
